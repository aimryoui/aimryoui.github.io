...
function Tooltip({
    children,
    ...props
}: Omit<TooltipPrimitive.Root.Props<TooltipPayload>, "children" | "handle"> & {
    children?: React.ReactNode
}) {
    const [handle] = useState(TooltipPrimitive.createHandle<TooltipPayload>)

    return (
        <TooltipHandleContext.Provider value={handle}>
            {children}
            <TooltipPrimitive.Root<TooltipPayload>
                data-slot="tooltip"
        ...
        <TooltipPrimitive.Portal>
            ...
            >
                <TooltipPrimitive.Popup
                    ...
                    className={cn(
                        "relative h-[--popup-height,auto] w-[--popup-width,auto] max-w-125 origin-[--transform-origin] text-balance rounded-xl bg-background text-sm tracking-tight text-foreground -outline-offset-px outline-muted-foreground will-change-[width,height,opacity,transform] outline transition-[width,height,opacity,transform] ease-spring duration-400",
                        {
