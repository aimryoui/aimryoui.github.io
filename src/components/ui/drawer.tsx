...
function DrawerOverlay({
    ...
    return (
        <DrawerPrimitive.Backdrop
            ...
            className={cn(
                "fixed inset-0 z-40 min-h-dvh select-none bg-black/30 opacity-[max(var(--drawer-overlay-min-opacity,0),calc(1-var(--drawer-swipe-progress)))] transition-opacity ease-[cubic-bezier(0.32,0.72,0,1)] duration-450 data-starting-style:opacity-0 data-ending-style:pointer-events-none data-ending-style:opacity-0 data-ending-style:duration-[calc(var(--drawer-swipe-strength)*400ms)] data-[swiping]:duration-0 data-[snap-points]:[--drawer-overlay-min-opacity:0.5] supports-[-webkit-touch-callout:none]:absolute supports-[backdrop-filter]:backdrop-blur-sm",
                className
            )}
            {...props}
        />
    ...
    return (
        <DrawerPortal data-slot="drawer-portal">
            ...
            <DrawerPrimitive.Viewport
                ...
                data-modal={modal}
                className="pointer-events-none fixed inset-0 z-40 select-none data-[modal=true]:pointer-events-auto"
            >
                <DrawerPrimitive.Popup
                    ...
                    className={cn(
                        ...
                        // Bleed.
                        "after:pointer-events-none after:absolute after:bg-[--drawer-bleed-background,var(--color-popover)] data-[swipe-axis=x]:after:inset-y-0 data-[swipe-axis=y]:after:inset-x-0 data-[swipe-direction=down]:after:top-full data-[swipe-direction=left]:after:right-full data-[swipe-direction=right]:after:left-full data-[swipe-direction=up]:after:bottom-full data-[swipe-axis=y]:after:h-[--bleed] data-[swipe-axis=x]:after:w-[--bleed]",
                        // Sizing.
                        "[--drawer-content-height:--drawer-height,auto] data-[swipe-axis=y]:data-[snap-points]:[--drawer-content-height:100dvh] data-[swipe-axis=y]:[--drawer-content-max-height:calc(100dvh-6rem)] data-[swipe-axis=x]:[--drawer-content-width:75%] data-[swipe-axis=x]:sm:[--drawer-content-width:24rem]",
                        // Stack.
                        "[--bleed:3rem] [--peek:1rem] [--stack-height:--drawer-frontmost-height,var(--drawer-height,0px)] [--stack-peek-offset:max(0px,calc((var(--nested-drawers)-var(--stack-progress))*var(--peek)))] [--stack-progress:clamp(0,var(--drawer-swipe-progress),1)] [--stack-scale-base:max(0,calc(1-(var(--nested-drawers)*var(--stack-step))))] [--stack-scale:clamp(0,calc(var(--stack-scale-base)+(var(--stack-step)*var(--stack-progress))),1)] [--stack-shrink:calc(1-var(--stack-scale))] [--stack-step:0.05]",
                        // Transitions.
                        "data-starting-style:transform-[--closed-transform] data-ending-style:opacity-[0.9999] data-ending-style:transform-[--closed-transform] data-ending-style:duration-[calc(var(--drawer-swipe-strength)*400ms)] data-[nested-drawer-swiping]:duration-0 data-[swiping]:duration-0 data-ending-style:data-[nested-drawer-swiping]:duration-[calc(var(--drawer-swipe-strength)*400ms)] data-ending-style:data-[swiping]:duration-[calc(var(--drawer-swipe-strength)*400ms)]",
                        // Axis: y.
...
function DrawerHeader({ className, ...props }: React.ComponentProps<"div">) {
    return (
        <div
            ...
            className={cn(
                "flex shrink-0 flex-col gap-1.5 p-4 pb-0 text-left group-data-[swipe-axis=y]/drawer-popup:text-center md:gap-0.5",
                className
            )}
            {...props}
        />
